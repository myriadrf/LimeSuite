...
void dlgADPDControls::createControls()
{
	...
	if ((m_iValue & 0x00003) == 0x00000)  { plotType->SetSelection(0); m_iPlotOpt = 0; }
	else if ((m_iValue & 0x00003) == 0x00001) { plotType->SetSelection(1); m_iPlotOpt = 1; }
	else if ((m_iValue & 0x00003) == 0x00002) { plotType->SetSelection(2); m_iPlotOpt = 2; }
	...
	if (m_iPlotOpt==0) td_xpI->Enable(true);
	else td_xpI->Enable(false);
	if ((m_iValue & 0x00004) == 0x00004) td_xpI->SetValue(true);  // bit 2
	else td_xpI->SetValue(false);	
	...
	if (m_iPlotOpt == 0) td_xpQ->Enable(true);
	else td_xpQ->Enable(false);
	if ((m_iValue & 0x00008) == 0x00008) td_xpQ->SetValue(true);  // bit 3
	else td_xpQ->SetValue(false);	
	...
	if (m_iPlotOpt == 0) td_ypI->Enable(true);
	else td_ypI->Enable(false);
	if ((m_iValue & 0x00010) == 0x00010) td_ypI->SetValue(true);  // bit 4
	else td_ypI->SetValue(false);
	...
	if (m_iPlotOpt == 0) td_ypQ->Enable(true);
	else td_ypQ->Enable(false);
	if ((m_iValue & 0x00020) == 0x00020) td_ypQ->SetValue(true);  // bit 5
	else td_ypQ->SetValue(false);	
	...
	if (m_iPlotOpt == 0) td_xI->Enable(true);
	else td_xI->Enable(false);
	if ((m_iValue & 0x00040) == 0x00040) td_xI->SetValue(true);  // bit 6
	else td_xI->SetValue(false);
	...
	if (m_iPlotOpt == 0) td_xQ->Enable(true);
	else td_xQ->Enable(false);
	if ((m_iValue & 0x00080) == 0x00080) td_xQ->SetValue(true);  // bit 7
	else td_xQ->SetValue(false);
	...
	if (m_iPlotOpt == 0) td_yI->Enable(true);
	else td_yI->Enable(false);
	if ((m_iValue & 0x00100) == 0x00100) td_yI->SetValue(true);  // bit 8
	else td_yI->SetValue(false);
	...
	if (m_iPlotOpt == 0) td_yQ->Enable(true);
	else td_yQ->Enable(false);
	if ((m_iValue & 0x00200) == 0x00200) td_yQ->SetValue(true);  // bit 9
	else td_yQ->SetValue(false);
	...
	td_uI->Show(true);
	if (m_iPlotOpt == 0) td_uI->Enable(true);
	else td_uI->Enable(false);
	if ((m_iValue & 0x00400) == 0x00400) td_uI->SetValue(true);  // bit 10
	else td_uI->SetValue(false);
	...
	if (m_iPlotOpt == 0) td_uQ->Enable(true);
	else td_uQ->Enable(false);
	if ((m_iValue & 0x00800) == 0x00800) td_uQ->SetValue(true);  // bit 11
	else td_uQ->SetValue(false);
	...
	if (m_iPlotOpt == 0) td_error->Enable(true);
	else td_error->Enable(false);
	if ((m_iValue & 0x01000) == 0x01000) td_error->SetValue(true);  // bit 12
	else td_error->SetValue(false);
	...
	if (m_iPlotOpt == 0) td_phase_error->Enable(true);
	else td_phase_error->Enable(false);
	if ((m_iValue & 0x02000) == 0x02000) td_phase_error->SetValue(true);  // bit 13
	else td_phase_error->SetValue(false);
	...
	if (m_iPlotOpt == 1) IvsQ->Enable(true);
	else IvsQ->Enable(false);
	if ((m_iValue & 0x0C000) == 0x00000) IvsQ->SetSelection(0); // bits 15-14
	else if ((m_iValue & 0x0C000) == 0x04000) IvsQ->SetSelection(1);
	else if ((m_iValue & 0x0C000) == 0x08000) IvsQ->SetSelection(2);
	...
	if (m_iPlotOpt == 2) fft_yp->Enable(true);
	...
	if (m_iPlotOpt == 2) fft_x->Enable(true);
	...
	if (m_iPlotOpt == 2) fft_y->Enable(true);
...
void dlgADPDControls::onEnableDisable()
{
	...
	if (temp == 0) { // time domain options enabled		
		td_xpI->Enable(true);
		td_xpQ->Enable(true);
		td_ypI->Enable(true);
		td_ypQ->Enable(true);
		...
		if (td_xpI->IsChecked()) m_iValue |= 0x00004;  // bit 2
		else m_iValue &= 0xFFFFB; 
		if (td_xpQ->IsChecked()) m_iValue |= 0x00008;  // bit 3
		else m_iValue &= 0xFFFF7;
		if (td_ypI->IsChecked()) m_iValue |= 0x00010;  // bit 4
		else m_iValue &= 0xFFFEF;
		if (td_ypQ->IsChecked()) m_iValue |= 0x00020;  // bit 5
		else m_iValue &= 0xFFFDF;
		if (td_xI->IsChecked()) m_iValue |= 0x00040;  // bit 6
		else m_iValue &= 0xFFFBF;
		if (td_xQ->IsChecked()) m_iValue |= 0x00080;  // bit 7
		else m_iValue &= 0xFFF7F;
		if (td_yI->IsChecked()) m_iValue |= 0x00100;  // bit 8
		else m_iValue &= 0xFFEFF;
		if (td_yQ->IsChecked()) m_iValue |= 0x00200;  // bit 9
		else m_iValue &= 0xFFDFF;
		if (td_uI->IsChecked()) m_iValue |= 0x00400;  // bit 10
		else m_iValue &= 0xFFBFF;
		if (td_uQ->IsChecked()) m_iValue |= 0x00800;  // bit 11
		else m_iValue &= 0xFF7FF;
		if (td_error->IsChecked()) m_iValue |= 0x01000;  // bit 12
		else m_iValue &= 0xFEFFF;
	...
	else if (temp == 1) {  // IQ constellation options  enabled
		td_xpI->Enable(false);
		td_xpQ->Enable(false);
		td_ypI->Enable(false);
		td_ypQ->Enable(false);
		...
		if (fft_xp->IsChecked()) m_iValue |= 0x10000;  // bit 16
		else m_iValue &= 0xEFFFF;
		if (fft_yp->IsChecked()) m_iValue |= 0x20000;  // bit 17
		else m_iValue &= 0xDFFFF;
		if (fft_x->IsChecked()) m_iValue |= 0x40000;  // bit 18
		else m_iValue &= 0xBFFFF;
