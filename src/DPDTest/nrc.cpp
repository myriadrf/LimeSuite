...
/*************************************************/
void nrc::nrerror(char *error_text)
{
...
int nrc::ludcmp(double **a, int n, int *indx, double *d1)
{
	...
	*d1=1.0;
	...
	if (exit == 1) return -1; // Singular matrix
...
//	double *g = new double[n+1];
	...
	for(iter=0; iter<ITMAX; iter++) {	
		...
		for(int i=1; i<=n; i++) {
			xx = x[i] + alpha*b[i];
			for(int j=1; j<=n; j++) xx -= alpha*a[i][j]*x[j];
			e += fabs(x[i] - xx);
			x[i] = xx;
		}

		if( (e < TOL) && (iter > ITMIN) ) break;		
	}
...
void nrc::gauss_seidel(double **a, double *b, double *x, int n)
{
	...
	for(iter=0; iter<ITMAX; iter++) {	
		...
		for(int i=1; i<=n; i++) {
			xx = b[i];
			for(int j=1; j<=n; j++) if(j != i) xx -= a[i][j]*x[j];
