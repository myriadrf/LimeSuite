...
API_EXPORT int CALL_CONV LMS_LoadConfig2(lms_device_t *device, const char *filename, int ind)
{
    if (device == nullptr)
    {
        lime::ReportError("Device cannot be NULL.");
    ...
    return lms->LoadConfig(filename, ind);
}
...
API_EXPORT int CALL_CONV SPI_write(lms_device_t * lmsControl, uint16_t address, uint16_t data) {
	...
	int ret = 0;
	if (LMS_WriteFPGAReg(lmsControl, address, data) != 0) ret = -1;
	return ret;
}
...
API_EXPORT void CALL_CONV UpdateHannCoeff(lms_device_t * lmsControl, uint16_t Filt_N, int chAA)
{
	Register reg, reg2;
	uint16_t msb, lsb = 0;
	uint16_t data = 0;
	...
	if (chAA == 1) reg = Register(0x0045, 0, 0, 0, 0); // chkSLEEP_CFR
	else reg = Register(0x0045, 8, 8, 0, 0); // chkSLEEP_CFR_chB

	if (chAA == 1) reg2 = Register(0x0045, 2, 2, 0, 0); //chkODD_CFR
	else reg2 = Register(0x0045, 10, 10, 0, 0);  //  chkODD_CFR_chB
	...
	msb = lsb = 0;
	i = j = 0;
	offset = 20 - ((uint16_t)((Filt_N + 1) / 2));
	while (i < Filt_N) {
		addr = (2 << 15) + (maddressf0 << 6) + (msb << 2) + lsb;

		if (j >= offset) data = w[i];
		else  data = 0;
	...
	if ((Filt_N % 2) == 1)  SetRegValue(lmsControl, reg2, 1); // odd
	else   SetRegValue(lmsControl, reg2, 0); // even
...
API_EXPORT int LoadQSparkSettings(lms_device_t * device, const char* filename)
{
	...
	if (parser.select("LimeSDR_QPCIe"))
	{
        ...
        
        {
			...
			//if (FreqRxMHz > 0 && FreqTxMHz > 0)
			//	lms->SetRate(4, FreqRxMHz*1e6, FreqTxMHz*1e6);
        ...
        //double FreqRxMHz=  30.72;
        //double FreqTxMHz=  30.72;
        //lms->SetRate(4, FreqRxMHz*1e6, FreqTxMHz*1e6);
		...
		//reg3= Register(0x004A, 7, 0, 0, 0);   // dummy, CFR_ORDER
		//LMS_ReadFPGAReg(device, reg3.address, &regValue); // read bit content
        int CFR_N_chA = parser.get<string, int>("CFR_N_chA", 19);		
		UpdateHannCoeff(device, CFR_N_chA, 1);
		...
		//reg3 = Register(0x004A, 15, 8, 0, 0);   // dummy, CFR_ORDER_chB
		//LMS_ReadFPGAReg(device, reg3.address, &regValue); // read bit content	
        int CFR_N_chB = parser.get<string, int>("CFR_N_chB", 19);	
		UpdateHannCoeff(device, CFR_N_chB, 0);
		return 0;
	}
	return -1;
}
