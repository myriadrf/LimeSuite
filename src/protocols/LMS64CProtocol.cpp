...
void LMS64CPacketMemoryWriteView::SetData(const std::byte* src, size_t len)
{
    assert(len <= 32);
    len = len > 32 ? 32 : len;
...
void LMS64CPacketMemoryWriteView::GetData(std::byte* dest, size_t len) const
{
    assert(len <= 32);
    len = len > 32 ? 32 : len;
    ...
    uint32_t subDevice)
{
    ...
    while (srcIndex < count)
    {
        ...
        sent = port.Write(reinterpret_cast<std::byte*>(&pkt), sizeof(pkt), 100);
        if (sent != sizeof(pkt))
            return -1;

        recv = port.Read(reinterpret_cast<std::byte*>(&pkt), sizeof(pkt), 1000);
...
int GetFirmwareInfo(ISerialPort& port, FirmwareInfo& info, uint32_t subDevice)
{
    ...
    pkt.subDevice = subDevice;
    int sent = port.Write(reinterpret_cast<std::byte*>(&pkt), sizeof(pkt), 100);
    if (sent != sizeof(pkt))
    {
        return -1;
    }

    int recv = port.Read(reinterpret_cast<std::byte*>(&pkt), sizeof(pkt), 1000);
    if (recv != sizeof(pkt) || pkt.status != STATUS_COMPLETED_CMD)
    {
        return -1;
    }
...
int I2C_Write(ISerialPort& port, uint32_t address, const std::byte* mosi, size_t count)
{
    return -1;
}
...
int CustomParameterWrite(ISerialPort& port, const std::vector<CustomParameterIO>& parameters, uint32_t subDevice)
{
    ...
    while (index < parameters.size())
    {
        ...
        int recv = port.Read(reinterpret_cast<std::byte*>(&pkt), sizeof(pkt), 100);
    ...
    uint32_t subDevice)
{
    ...
    for (uint32_t chunkIndex = 0; chunkIndex < chunkCount && !abortProgramming; ++chunkIndex)
    {
        ...
        if (needsData)
        {
            progView.SetData(data, chunkSize);
            data += chunkSize;
        }

        if (port.Write(reinterpret_cast<std::byte*>(&packet), sizeof(packet), progTimeout_ms) != sizeof(packet))
        {
            if (callback)
                callback(bytesSent, length, "Programming failed! Write operation failed");
            return false;
        }
        if (port.Read(reinterpret_cast<std::byte*>(&inPacket), sizeof(inPacket), progTimeout_ms) != sizeof(inPacket))
        {
            if (callback)
...
int GPIODirRead(ISerialPort& port, std::byte* buffer, const size_t bufLength)
{
    ...
    int bytesSent = port.Write(reinterpret_cast<std::byte*>(&pkt), sizeof(pkt), 100);
    if (bytesSent != sizeof(pkt))
    {
        throw std::runtime_error("GPIODirRead write failed");
    }

    int bytesReceived = port.Read(reinterpret_cast<std::byte*>(&pkt), sizeof(pkt), 100);
    ...
    {
        buffer[i] = std::byte{ pkt.payload[i] };
    }
...
int GPIODirWrite(ISerialPort& port, const std::byte* buffer, const size_t bufLength)
{
    ...
    for (size_t i = 0; i < bufLength; ++i)
    {
        pkt.payload[i] = std::to_integer<uint8_t>(buffer[i]);
    }
...
int MemoryWrite(ISerialPort& port, uint32_t address, const std::byte* data, size_t dataLen, uint32_t subDevice)
{
    ...
    for (uint32_t chunkIndex = 0; chunkIndex < chunkCount; ++chunkIndex)
    {
        ...
        if (port.Write(reinterpret_cast<std::byte*>(&packet), sizeof(packet), timeout_ms) != sizeof(packet))
            return -1;
        if (port.Read(reinterpret_cast<std::byte*>(&inPacket), sizeof(inPacket), timeout_ms) != sizeof(inPacket))
            return -1;
...
int MemoryRead(ISerialPort& port, uint32_t address, std::byte* data, size_t dataLen, uint32_t subDevice)
{
    const int timeout_ms = 100;
    size_t bytesGot = 0;
    std::byte* dest = data;
