...
# Open port
if os.name == 'nt':
    fd = libLimeSuite.LIMERFE_Open(c_char_p("COM17"), c_int(baudrate));
else:
...
libLimeSuite.LIMERFE_Configure(c_int(0), c_int(fd), c_int(LH.CID_HAM_0145), c_int(LH.CID_HAM_0145), c_int(LH.PORT_1), c_int(LH.PORT_1), c_int(LH.MODE_RX), c_int(LH.NOTCH_VALUE_OFF), c_int(0))

time.sleep(1.0)

# Change mode to transmit
libLimeSuite.LIMERFE_Mode(c_int(0), c_int(fd), c_int(LH.MODE_TX))

time.sleep(1.0)

# Reset LimeRFE
libLimeSuite.LIMERFE_Reset(c_int(0), c_int(fd))
