...
int ConnectionXillybus::ResetStreamBuffers()
{
    if (hWriteStream[0] >= 0)
        close(hWriteStream[0]);

    if ((hWriteStream[0] = open(writeStreamPort[0].c_str(), O_WRONLY | O_NOCTTY | O_NONBLOCK))==-1)
    {
        ReportError(errno);
        return -1;
    }
...
int ConnectionXillybus::ReadDPDBuffer3(char* buffer, unsigned length)
{
...
//	StartStreaming();
	...
	//addrs.push_back(0x0040); values.push_back(length / 20);
        //addrs.push_back(0x0041); values.push_back(0x00); //values.push_back(0);
	//this->WriteRegisters(addrs.data(), values.data(), values.size());
	//addrs.clear(); values.clear();
	//addrs.push_back(0x0041); values.push_back(0x01); //values.push_back(1);
	//this->WriteRegisters(addrs.data(), values.data(), values.size());
...
int ConnectionXillybus::ReadDPDBuffer2(char* buffer, unsigned length)
{
	
	//switch off Rx/Tx
	uint16_t interface_ctrl_000A;
	ReadRegister(0x000A, interface_ctrl_000A);
	WriteRegister(0x000A, interface_ctrl_000A & ~0x1);
	WriteRegister(0x0007, 1);
	...
	ReadRegister(0x0008, interface_cfg_0008);
	interface_cfg_0008 = (interface_cfg_0008 & ~0x3) | smpl_width;
	WriteRegister(0x0008, interface_cfg_0008);
	...
	//int handle = BeginDataReading((char*)buffer, bytesToRead);
	//if (WaitForReading(handle, 1000) != 1)
	//{
	//	wxMessageBox("Failed to receive data");
	//}

        WriteRegister(0x0040, length / 20);
        uint16_t interface_cfg_0041;
        ReadRegister(0x0041, interface_cfg_0041);
	interface_cfg_0041 = (interface_cfg_0041 & ~0x1); // reset lsb
        WriteRegister(0x0041, interface_cfg_0041);
	...
	long btr = length;
	int bytesReceived = FinishDataReading((char*)buffer, btr, 2);
	AbortReading(2);
...
// Borko 29.01.2019
int ConnectionXillybus::StartStreaming()
{
	uint32_t value;
	int interface_ctrl_000A = ReadRegister(0x000A, value);
...
// Borko 29.01.2019
int ConnectionXillybus::StopStreaming()
{
	uint32_t value;
	int interface_ctrl_000A = ReadRegister(0x000A, value);
	if (interface_ctrl_000A < 0)
