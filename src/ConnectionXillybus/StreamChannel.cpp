...
ConnectionXillybus::StreamChannel::StreamChannel(lime::IConnection* port) :
    mActive(false)
{
    this->port = dynamic_cast<ConnectionXillybus*>(port);
    fifo = new RingFIFO(1024*8);
}

ConnectionXillybus::StreamChannel::~StreamChannel()
{
    delete fifo;
}

int ConnectionXillybus::StreamChannel::Read(void* samples, const uint32_t count, Metadata* meta, const int32_t timeout_ms)
{
    int popped = 0;
...
int ConnectionXillybus::StreamChannel::Write(const void* samples, const uint32_t count, const Metadata *meta, const int32_t timeout_ms)
{
    int pushed = 0;
    if(config.format == StreamConfig::STREAM_COMPLEX_FLOAT32 && config.isTx)
    {
...
IStreamChannel::Info ConnectionXillybus::StreamChannel::GetInfo()
{
    Info stats;
...
bool ConnectionXillybus::StreamChannel::IsActive() const
{
    return mActive;
}

int ConnectionXillybus::StreamChannel::Start()
{
    mActive = true;
    fifo->Clear();
...
int ConnectionXillybus::StreamChannel::Stop()
{
    mActive = false;
    return port->UpdateThreads();
}
