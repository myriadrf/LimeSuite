...
void lms7002_pnlRxTSP_view::UpdateNCOinputs()
{
    ...
    if (rgrMODE_RX->GetSelection() == 0) //FCW mode        
    {
        ...
        {
            txtNCOinputs[i]->SetValue(wxString::Format(_("%.6f"), lmsControl->GetNCOFrequency_MHz(LMS7002M::Rx, i)));
        }
        txtFCWPHOmodeAdditional->SetValue(wxString::Format(_("%i"), lmsControl->SPI_read(0x0441)));
    ...
    else //PHO mode
    {
        ...
        txtFCWPHOmodeAdditional->SetValue(wxString::Format(_("%.6f"), lmsControl->GetNCOFrequency_MHz(LMS7002M::Rx, 0)));
