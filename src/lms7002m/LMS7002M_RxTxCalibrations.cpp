...
int LMS7002M::CalibrateTxSetup(float_type bandwidth_Hz, const bool useExtLoopback)
{
    ...
    if(useExtLoopback)
    {
        ...
        Modify_SPI_Reg_bits(CFB_TIA_RFE, 700); //to filter aliasing
...
{
    const bool rxDC = args->param.address == DCOFFI_RFE.address;
    uint32_t rssiLeft = ~0;
    uint32_t rssiRight;
...
'-' u 1:2 with lines title 'target Level'\n");
    ...
    uint32_t rssi = GetRSSI();
    int8_t g_pga = Get_SPI_Reg_bits(G_PGA_RBB);
    int8_t g_rxloop = Get_SPI_Reg_bits(G_RXLOOPB_RFE);
...
void LMS7002M::CalibrateIQImbalance(const bool tx, uint16_t *gainI, uint16_t *gainQ, int16_t *phase)
{
    ...
    const auto gcorri = tx ? GCORRI_TXTSP : GCORRI_RXTSP;
    const auto gcorrq = tx ? GCORRQ_TXTSP : GCORRQ_RXTSP;
    int16_t gain;
    int16_t phaseOffset;
    ...
    argsPhase.param = tx ? IQCORR_TXTSP : IQCORR_RXTSP;
