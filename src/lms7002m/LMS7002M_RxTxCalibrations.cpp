...
#include "Logger.h"
#include "LMSBoards.h"
//#include "../../mcu_program/host_src/mcuHexBin.h"
...
int LMS7002M::CalibrateTx(float_type bandwidth_Hz, bool useExtLoopback)
{
    ...
    if(mcuID != MCU_ID_CALIBRATIONS_SINGLE_IMAGE)
    {
        ...
        uint16_t imgSize = 0;
        status = mcuControl->GetProgramCode("calibrationsLMS7_MCU.hex", false);
        if(status != 0)
            return status;
        
        verbose_printf("Uploading DC/IQ calibration firmware\n");
        status = mcuControl->Program_MCU(mcuControl->byte_array, IConnection::MCU_PROG_MODE::SRAM);
        if(status != 0)
            return status;
    }
      * */
...

    {
        ...
        status = mcuControl->WaitForMCU(2000);
