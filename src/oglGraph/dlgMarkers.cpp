...
void dlgMarkers::AddMarker(int id)
{
	...
	wxStaticText* StaticText4 = new wxStaticText(pnl, markerId, _T("0.0"), wxDefaultPosition, wxSize(64,-1), 0, _T("ID_STATICTEXT4"));
	labels.push_back(StaticText4);
	marker_values.push_back(0);
...
void dlgMarkers::AddDeltas()
{
    ...
    for(int i=0; i<5; ++i)
    {
        ...
        wxStaticText* lblDelta0 = new wxStaticText(pnl, wxNewId(), _T("0.0 dB"), wxDefaultPosition, wxSize(74,-1), 0, _T("ID_STATICTEXT2"));
    ...
    if(IsShownOnScreen())
    {
        for(int i=0; i<labels.size(); ++i)
        {
            if(parent_graph == NULL)
                return;
            double value = parent_graph->markers[i].posY;
            double freq =  parent_graph->markers[i].posX/1000000;
            labels[i]->SetLabel(wxString::Format("%3.1f", value));
            marker_values[i] = value;            
        ...
        for(int i=0; i<5; ++i)
        {
            ...
            deltaValue = marker_values[src1]-marker_values[src2];
            deltaValues[i]->SetLabel(wxString::Format("%.3f dB", deltaValue));
        }
    }
}
