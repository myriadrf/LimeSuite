...
def siggen_app(
    ...
    while True:
        ...
        totalSamps += sr.ret
        if totalSamps > 3*rate:
            sampsCh0 = np.zeros(streamMTU).astype(np.complex64)
            sr = sdr.writeStream(txStream, [sampsCh0], sampsCh0.size, flags=SOAPY_SDR_END_BURST)
            if sr.ret != sampsCh0.size:
                raise Exception("Expected writeStream(SOAPY_SDR_END_BURST) to consume all samples! %d"%sr.ret)
            time.sleep(3)
            totalSamps = 0
            print 'slept'
